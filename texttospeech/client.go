...
// Client provides ElevenLabs text-to-speech APIs.
type Client struct {
	core *elevenlabs.Client
}

// NewClient creates a text-to-speech client that authenticates with apiKey.
func NewClient(apiKey string, opts ...elevenlabs.ClientOption) *Client {
	return New(elevenlabs.NewClient(apiKey, opts...))
}

// New creates a text-to-speech client from a shared ElevenLabs client.
