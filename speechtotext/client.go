...
// Client provides ElevenLabs speech-to-text APIs.
type Client struct {
	core *elevenlabs.Client
}

// NewClient creates a speech-to-text client that authenticates with apiKey.
func NewClient(apiKey string, opts ...elevenlabs.ClientOption) *Client {
	return New(elevenlabs.NewClient(apiKey, opts...))
}

// New creates a speech-to-text client from a shared ElevenLabs client.
