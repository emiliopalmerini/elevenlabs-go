...
// DoStream executes requests with the client's retry policy and returns the
// successful response body without reading it. The caller must close the body.
func (c *Client) DoStream(ctx context.Context, build RequestBuilder, retryable bool) (io.ReadCloser, RawResponse, error) {
	if c == nil {
		return nil, RawResponse{}, errors.New("elevenlabs: nil client")
	}
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if resp.StatusCode >= 200 && resp.StatusCode < 300 {
			return resp.Body, newRawResponse(resp), nil
		}
